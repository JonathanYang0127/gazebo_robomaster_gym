...
def generate_obstacle_segments(buffer):
    ...
    for xl, xr, yb, yt in parallel_obstacles:
        ...
        segments.extend([(xl, yb, xl, yt), (xl, yt, xr, yt), (xr, yt, xr, yb), (xr, yb, xl, yb)])
    left, hmid, right, bot, vmid, top = center_obstacle
    left, right, top, bot = left - buffer, right + buffer, top + buffer, bot - buffer
    
    return segments + [(left, vmid, hmid, top), (hmid, top, right, vmid), (right, vmid, hmid, bot), (hmid, bot, left, vmid)]
...
def seg_intersect(x1, y1, x2, y2, x3, y3, x4, y4):
    a1,a2,b1,b2 = np.array([x1, y1]),np.array([x2, y2]),np.array([x3, y3]),np.array([x4, y4])
    da = a2-a1
    db = b2-b1
    dp = a1-b1
    dap = perp(da)
