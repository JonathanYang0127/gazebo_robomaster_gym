...
class RobomasterEnv(gym.Env):
    """ Implementation notes:
        children MUST implement update_robot_coords and pub_cmds 
        robot_coord must be of form (x,y,yaw)
    """
    def __init__(self, statistics_gui=False):
        # Set up state parameters
        self.damage_threshold = 6

        self._robot_coords = [None] * 4
        self._robot_plates_coords = [None] * 4

        self._running_step = 0.1 # Amount of time running per timestep
        self._max_timesteps = 1000 # Max number of timesteps
        self._real_time_conversion = 1 # Conversion from timestep to real time
    ...
    def step(self, actions): 
        ...
        for robot_index in range(4):
            if self._robot_coords[robot_index] is not None:
        ...
        for i in range(4):
            self.pub_cmds(i, actions[i], actions[i][-1])
            self.update_robot_coords(i)

        for i in range(4):
            if actions[i][3] and self._num_projectiles[i] > 0:
                robot, plate = self.get_hit(i)
                if robot >= 0:
                    self._robot_hp[robot] -= 60 if plate == 3\
                                        else 50 if plate == 2 or plate == 1\
                                        else 30
    ...
    def pub_cmds(self, _id, vel, gimbal):
    ...
    def _on_init(self):
        ...
        self.cmd_vel = [Point(0,0,0) for i in range(4)]
        self.cmd_gimbal = [0 for i in range(4)]
    ...
    def is_line_of_sight_blocked(self, x1, y1, x2, y2, from_robot_index=None):
        # uninitiated
        if not self._robot_coords:
            return True
        for robot_index in range(4):
            if robot_index != from_robot_index:
                _x1, _y1, _x2, _y2, _x3, _y3, _x4, _y4 = self._robot_coords[robot_index]
    ...
    def get_best_enemy_plate(self, robot_index):
        ...
        for enemy in self.get_enemy_team(robot_index):
            for plate_index in range(4):
                x1, y1, x2, y2 = self._robot_plates_coords[enemy][plate_index]
    ...
    def get_state(self):
        ...
        robot_state = [self._robot_coords[i][:] + [self._num_projectiles[i], self._barrel_heat[i], self._robot_hp[i]]
                       for i in range(4)]
